...
#显示图片路径以及识别结果的窗口
tkinter.Label(window, text='图片路径为: ', font=("微软雅黑", 20)).place(x=40, y=60)
tkinter.Label(window, text='图片识别结果为: ', font=("微软雅黑", 20)).place(x=40, y=120)


var_load = tkinter.StringVar()
entry_load = tkinter.Entry(window, textvariable=var_load, font=("微软雅黑", 15))
entry_load.place(x=250, y=65, width=200, height=30)
var_s = tkinter.StringVar()
entry_s = tkinter.Entry(window, textvariable=var_s, font=("微软雅黑", 15))
entry_s.place(x=250, y=125, width=200, height=30)
'''
...
submit_button = tkinter.Button(window, text="选择图片", font=("微软雅黑", 20), command=choose_fiel).place(x=100, y=230)
submit_button = tkinter.Button(window, text="图片识别", font=("微软雅黑", 20), command=lambda: main(var_load.get())).place(x=250, y=230)
